...
ps2_bottom_half:
	...
	# Do three different things based on keyPRESS
	pushq	%rbx
	movzbq	(read_bytes), %rbx
	bt	$7, %rbx
	jc	3f
	movzbq	(gameState), %rbx
	cmpq	$1, %rbx
	jne	1f
		# If in game
		call	setDir	# set direction for snake
		jmp	3f
	1:
	cmpq	$0, %rbx
	jne	2f
		# If in titleScreen
		call	startGame	# go from beginning screen to game
		jmp	3f
	2:
	cmpq	$2, %rbx
	jne	3f
		# If in game over screen
		call	startScreen	# go to startscreen again
		jmp	3f
	3:
	popq	%rbx
